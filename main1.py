...
class form(QMainWindow):
    ...
    def delrow(self):
        if self.ui.tableWidget.currentIndex().row() > -1:
            ...
            QMessageBox.question(self,'Message', "Выбери строку, которую желаешь удалить!", QMessageBox.Ok)
            self.show()
    ...
    def updaterow(self):
        if self.ui.tableWidget.currentIndex().row() > -1:
            ...
            record.setValue("DATE", self.ui.dateEdit.text())
            record.setValue("Phone", self.ui.lineEdit_3.text())
            self.model.setRecord(self.ui.tableWidget.currentIndex().row(), record)
        else:
            QMessageBox.question(self,'Message', "Выбери строку, которую желаешь изменить", QMessageBox.Ok)
            self.show()
